...
try {
//on teste si le nouvel utilisateur est un pilote ou un club et on enregistre les données en fonction
if ($_POST['typeUser']=="pilote"){
    $mail=htmlspecialchars($_POST['emailPilote'], ENT_QUOTES);
    $pass=htmlspecialchars($_POST['pwdPilote'], ENT_QUOTES);
    $civilite=htmlspecialchars($_POST['civilite'], ENT_QUOTES);
    $nom=htmlspecialchars($_POST['nomPilote'], ENT_QUOTES);
    $prenom=htmlspecialchars($_POST['prenomPilote'], ENT_QUOTES);
    ...
    $sql = $sql . "VALUES ('".$licence."','".$civilite."','".$nom."','".$prenom."','".$naissance."','".$adresse."','".$tel."','";
    $sql = $sql .$contact."','".$tel_contact."','".$resp."',".$id_user.",".$id_ville.");";
    $resultat = $pdo->prepare($sql);
    $resultat->execute();
}
...
exit('Erreur enregistrement de utilisateur');
}
?>